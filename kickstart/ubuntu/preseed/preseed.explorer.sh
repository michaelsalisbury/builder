...
exp () {
	...
	local script="/tmp/interactive.${base}.sh"
	cat << END-OF-ENTRY >> "${file}"
cat << END-OF-INTERACTIVE > ${script}
# jump to tty6 to display message
exec < /dev/tty6 > /dev/tty6 2> /dev/tty6
chvt 6
echo Welcome to an interatice prompt after step \\\"${base}\\\"
echo
/bin/sh
chvt 1
...
analyze () {
	...
	msg 1 Free Space on \"${dev}\" :: $(in_M $(get_disk_free $dev))M
	...
	if ! has_free_space $dev $(to_G 10); then
		...
		msg 3 Drive empty\; Basic layout will be used on hard drive \"${dev}\".
		#cmd
	elif has_primary_part_free $dev 3; then
		debconf-set-selections /tmp/preseed/preseed.hd.free_extended.cfg
		msg 3 3 Free Primary Partitions\; Extended partition layout will be used on hard drive \"${dev}\".
		cmd
		...
		if has_extended_part $dev; then
			...
			debconf-set-selections /tmp/preseed/preseed.hd.free_extended.cfg
			msg 3 2 Free Primary Partitions\; Extended partition layout will be used on hard drive \"${dev}\".
			cmd
		fi
	elif has_primary_part_free $dev 1; then
		debconf-set-selections /tmp/preseed/preseed.hd.free_wo-swap.cfg
		debconf-set-selections /tmp/preseed/preseed.hd.partman_no-prompt.cfg
		msg 3 1 Free Primary Partition\; Layout without /boot, extended partition or swap will be used on hard drive \"${dev}\".
		cmd
	fi
}
explore () {
	local folder='/var/lib/dpkg/info'
	while read path; do
		exp ${path}
	done << END-OF-LIST
		`find ${folder} -type f -maxdepth 1 -name *postinst`
END-OF-LIST
}
analyze $@
#explore $@
