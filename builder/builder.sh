...
function include(){
	if [ -f "$1" ]; then
		...
		# Setup GLOBAL array to track sourced script
		declare -A includes

		# Include functions and global variables recursivelly if...
		`switch_set_default r X`
		local OPTIND=
		local OPTARG=
		local OPTION=
		local OPTERR=0
		# only include source recursivelly if eval_function requested
		while getopts "cei:nr:" OPTION; do
                	case $OPTION in
		...
		# if no options were passwd include source recursivelly
		#echo OPTIND :: $OPTIND
		#echo OPTARG :: $OPTARG
		#echo OPTION :: $OPTION
		(( OPTIND > 1 )) || include_file "${scriptFQFN}"
		return 0
	else
		return 1
	fi
