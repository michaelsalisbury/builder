...
function source_global_control_vars(){
	...
	local SHM && get_buildScriptSHMD SHM
...
function process_switches(){
	...
	while getopts "b:cCd:e:ghj:i:lm:nNr:s:t:uw:" OPTION; do
		...
		# process switches
			...
			i)	local index=`find_function $OPTARG`
				(( index )) && eval_function $index
				(( index )) && log_is_empty  $index;;
			...
			n)	skip || eval_function $step; next; wrap;;
			# Do Nothing
			N)	;;
	...
	(( ${#@} )) && for args in "$@"; do
		case ${switches_last_option:- null} in
			i|s|w)	$FUNCNAME -$switches_last_option "$args" -N;;
		esac
	done
	...
	if readlink /proc/$$/fd/0 | egrep -q "^pipe:"; then
		case ${switches_last_option:- null} in	
			i|s|w)	while read args; do
					$FUNCNAME -$switches_last_option "${args}" -N
	...
	# final action after lists
	case ${switches_last_option:- null} in
		i|s|n|j)	disp_functions
				echo;;
	esac
...
function include_file(){
	...
	local _INCLUDES_=GLOBAL_${FUNCNAME}_INCLUDES
	eval "${_INCLUDES_}=\"${!_INCLUDES_:- $'\n'}\""
...
function include_variables_bash(){
	...
	while read variable_name; do
		...
		if ${variable_is_available:-true}; then
			...
			-e 's/^[[:space:]]*//'\
			-e '/function[[:space:]]\+/,/^}/d'\
			-e '/.*read.*</s/.*read.*[[:space:]]\+\([a-zA-Z0-9_]\+\)[[:space:]]\+<.*/\1/p'\
			-e '/^declare/s/.*[[:space:]]\+\([a-zA-Z0-9_]\+\)\($\|=.*\)/\1/p'\
			-e '/^IFS[a-zA-Z0-9_]*=/d'\
			-e '/[^[:space:]]\+=/s/=.*//p'\
			-e 'd')
...
function log_buildScriptOutput(){ tee -a "$scrLogFQFN" >> "`log_get_name $1`"; }
function log_buildScriptError(){  tee >(log_buildScriptOutput $1) | sed "s|^|$1 :: $(name_function $1) :: |" >> "${scrLogFQFN}-error"; }
...
function eval_function(){
	eval `name_function $1`				\
		1> >(tee >(log_buildScriptOutput $1))	\
		2> >(tee >(log_buildScriptError  $1))
}
...
function disp_functions(){
	...
	REPC -
	# parse functions
	local name index desc toggle marker line
	while read name; do
		name=${name#${prefix}_}
		name=${name//_/ }
		...
		(( step == index ))	\
			...
			|| unset marker
		# format line
		line=`printf "${printf_format}" "${marker}" "${index}" "${toggle}" "${name}" "${desc}"`
		# print line and clip to screen width
...
###########################################################################################
###########################################################################################
 function get_buildScriptPipe(){
	local _PIPE_=GLOBAL_${FUNCNAME}_PIPE

	if [ -n "${!_PIPE_}" ] && [ -e "${!_PIPE_}" ]; then
		(( ${#1} ))			\
			&& eval $1="${!_PIPE_}"	\
			|| echo "${!_PIPE_}"
		return 0
	...
	local SHMD && get_buildScriptSHMD SHMD

	if [ -d "${SHMD}" ] && [ ! -e "${SHMD}/${buildScriptName}_PIPE" ]; then
		eval ${_PIPE_}="${SHMD}/${buildScriptName}_PIPE"
	elif [ -d "${SHMD}" ] && [ ! -e "${SHMD}/${buildScriptName}_PIPE$$" ]; then
		eval ${_PIPE_}="${SHMD}/${buildScriptName}_PIPE$$"
	else
		echo ERROR\!  Major problem setting up \"buildScriptPipe\". Exiting. 1>&2
		EXIT 1; exit 1
	fi
	mkfifo "${!_PIPE_}" &>/dev/null
	if (( $? )); then
		echo ERROR\!  Major problem creating \"buildScriptPipe\". Exiting. 1>&2
		EXIT 1; exit 1
	else
		(( ${#1} ))			\
			&& eval $1="${!_PIPE_}"	\
...
function get_buildScriptSHMD(){
	local _SHMD_=GLOBAL_${FUNCNAME}_SHMD

	if [ -n "${!_SHMD_}" ] && [ -d "${!_SHMD_}" ]; then
		(( ${#1} ))			\
			&& eval $1="${!_SHMD_}"	\
			|| echo "${!_SHMD_}"
		return 0
	elif [ -d "/dev/shm" ]; then
		eval ${_SHMD_}="/dev/shm/$$${buildScriptName}"
	elif [ -d "/tmp" ]; then
		eval ${_SHMD_}="/tmp/$$${buildScriptName}"
	elif (( ${#HOME} )) && [ -d "${HOME}" ]; then
		eval ${_SHMD_}="${HOME}/tmp-$$${buildScriptName}-SHMD"
	else
		echo ERROR\!  Major problem setting up \"buildScriptSHMD\". Exiting. 1>&2
		EXIT 1; exit 1
	fi
	mkdir -p "${!_SHMD_}" &>/dev/null
	if (( $? )); then
		echo ERROR\!  Problem creating dir \"buildScriptSHMD\". Exiting. 1>&2
		EXIT 1; exit 1
	else
		(( ${#1} ))			\
			&& eval $1="${!_SHMD_}"	\
...
function EXIT(){
	local SHMD=$(get_buildScriptSHMD)
	local PIPE=$(get_buildScriptPipe)
	rm -rf "${PIPE}"
	rm -rf "${SHMD}"
	exit ${1:- 0}
}
