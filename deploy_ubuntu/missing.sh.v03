...
function main(){
	...
	cat ${DEPENDENCIES} > "${SELECTIONS}"-dependencies
	...
	cat ${UNAVAILABLE}  > "${SELECTIONS}"-unavailable
	cat ${VIRTUAL_AUTO} > "${SELECTIONS}"-virtual_auto-select
	cat ${VIRTUAL_MAN}  > "${SELECTIONS}"-virtual_manual-select
	cat ${TROUBLE}      > "${SELECTIONS}"-trouble
	...
	cat ${MISSING}      > "${SELECTIONS}"-missing
	cat ${ADDONS}       > "${SELECTIONS}"-addons
	cat ${DEPENDENCIES} > "${SELECTIONS}"-dependencies
	...
	echo
	echo "TOTAL         :: ${TOTAL}"
	echo "TOTAL_MISSING :: ${TOTAL_MIS}"
	echo "TOTAL_DEPS    :: ${TOTAL_DEPS}"
	echo "TOTAL_TROUBLE :: ${TOTAL_SHOW}"
...
function PACKAGE_SHOW(){
	...
	apt-cache show ${PACKAGE} 2>/dev/null | grep "" &>/dev/null
	...
	(
		...
		for FILE in PACKAGES MISSING VIRTUAL_AUTO VIRTUAL_MAN UNAVAILABLE TROUBLE; do
			echo ${FILE} :: `wc -l ${!FILE}`
		done
	) | column -t
}
...
[ -f "$1" ] && SELECTIONS="$1" || {
	echo Bad or missing selections file.
	echo Generate via \"dpkg --get-selections \> some-file\".
	echo Exiting\!
	exit
}
...
diff --suppress-common-lines\
	<(awk '{print $1}' <(dpkg --get-selections))\
